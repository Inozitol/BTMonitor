...
void flow_analyzer::timeout_clear_start(uint32_t period) {
    timeout_task.push_back(std::async(std::launch::async, [period](){
        while(true){
            if(!thread_killer_t.wait_for(std::chrono::seconds(period))){
                clean_timeouts();
                break;
            }
            clean_timeouts();
...
void flow_analyzer::clean_timeouts(){
    ...
    for(auto it = flow_map.cbegin(); it != flow_map.cend();){
        ...
        if(timeout_check(it->second.packets.back().ts, now_micro)){
